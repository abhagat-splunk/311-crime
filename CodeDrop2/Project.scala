...
val combined_3 = sqlContext.sql("SELECT COUNT(`Unique Key`) as `Number of Parking Violations`,Zipcode, `HC01_VC03` as `Population` FROM combinedData where `Complaint Type`='Broken Parking Meter' OR `Complaint Type`='DOF Parking - Address Update' OR `Complaint Type`='DOF Parking - DMV Clearance' OR `Complaint Type`='DOF Parking - Payment Issue' OR `Complaint Type`='DOF Parking - Request Copy' OR `Complaint Type` = 'DOF Parking - Request Status' OR `Complaint Type` = 'DOF Parking - Tax Exemption' OR `Complaint Type`='Illegal Parking' OR `Complaint Type` = 'Municipal Parking Facility' group by Zipcode, `HC01_VC03`")